...
public:
    Rule_XPs_Spawn(Field* main_field) : field(main_field->get_field()) {};
    void operator()(T1 magic_number, T2 xps_amount, Field* main_field);
private:
    std::vector < std::vector<Cell> > field;
};

template<typename T1, typename T2>
void Rule_XPs_Spawn<T1, T2>::operator()(T1 magic_number, T2 xps_amount, Field *main_field) {
    field = main_field->get_field();

    for (int i = 1; i <= xps_amount; i++){
        while (true) {
